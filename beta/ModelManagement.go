// Code generated by msgraph.go/gen DO NOT EDIT.

package msgraph

// ManagementCertificateWithThumbprint undocumented
type ManagementCertificateWithThumbprint struct {
	// Object is the base model of ManagementCertificateWithThumbprint
	Object

	ODataType string `json:"@odata.type,omitempty"`
	// Certificate undocumented
	Certificate *string `json:"certificate,omitempty"`
	// Thumbprint undocumented
	Thumbprint *string `json:"thumbprint,omitempty"`
}

func NewManagementCertificateWithThumbprint() (*ManagementCertificateWithThumbprint, error) {
	newManagementCertificateWithThumbprint := &ManagementCertificateWithThumbprint{
		ODataType: "#microsoft.graph.ManagementCertificateWithThumbprint",
	}
	return newManagementCertificateWithThumbprint, nil
}
