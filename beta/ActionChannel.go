...
// ChannelMembersCollectionRequestBuilder is request builder for ConversationMember collection rcn
...
// ChannelMessagesCollectionRequestBuilder is request builder for ChatMessage collection rcn
...
// ChannelSharedWithTeamsCollectionRequestBuilder is request builder for SharedWithChannelTeamInfo collection rcn
type ChannelSharedWithTeamsCollectionRequestBuilder struct{ BaseRequestBuilder }

// Request returns request for SharedWithChannelTeamInfo collection
func (b *ChannelSharedWithTeamsCollectionRequestBuilder) Request() *ChannelSharedWithTeamsCollectionRequest {
	return &ChannelSharedWithTeamsCollectionRequest{
		BaseRequest: BaseRequest{baseURL: b.baseURL, client: b.client},
	}
}

// ID returns request builder for SharedWithChannelTeamInfo item
func (b *ChannelSharedWithTeamsCollectionRequestBuilder) ID(id string) *SharedWithChannelTeamInfoRequestBuilder {
	bb := &SharedWithChannelTeamInfoRequestBuilder{BaseRequestBuilder: b.BaseRequestBuilder}
	bb.baseURL += "/" + id
	return bb
}

// ChannelSharedWithTeamsCollectionRequest is request for SharedWithChannelTeamInfo collection
type ChannelSharedWithTeamsCollectionRequest struct{ BaseRequest }

// Paging perfoms paging operation for SharedWithChannelTeamInfo collection
func (r *ChannelSharedWithTeamsCollectionRequest) Paging(ctx context.Context, method, path string, obj interface{}, n int) ([]SharedWithChannelTeamInfo, error) {
	req, err := r.NewJSONRequest(method, path, obj)
	if err != nil {
		return nil, err
	}
	...
	var values []SharedWithChannelTeamInfo
	for {
		if res.StatusCode != http.StatusOK {
			b, _ := ioutil.ReadAll(res.Body)
			res.Body.Close()
...
// GetN performs GET request for SharedWithChannelTeamInfo collection, max N pages
func (r *ChannelSharedWithTeamsCollectionRequest) GetN(ctx context.Context, n int) ([]SharedWithChannelTeamInfo, error) {
	var query string
	if r.query != nil {
		query = "?" + r.query.Encode()
	}
...
// Get performs GET request for SharedWithChannelTeamInfo collection
func (r *ChannelSharedWithTeamsCollectionRequest) Get(ctx context.Context) ([]SharedWithChannelTeamInfo, error) {
	return r.GetN(ctx, 0)
}

// Add performs POST request for SharedWithChannelTeamInfo collection
func (r *ChannelSharedWithTeamsCollectionRequest) Add(ctx context.Context, reqObj *SharedWithChannelTeamInfo) (resObj *SharedWithChannelTeamInfo, err error) {
	err = r.JSONRequest(ctx, "POST", "", reqObj, &resObj)
	return
}
...
// ChannelTabsCollectionRequestBuilder is request builder for TeamsTab collection rcn
